...
namespace AnotherFileBrowser.Windows
{
    public class FileBrowser
    {
        ...
        public void OpenFileBrowser(BrowserProperties browserProperties, Action<string> filepath)
        {
            ...
            ofd.Title            = browserProperties.title ?? "Select a File";
            ofd.InitialDirectory = browserProperties.initialDir ?? @"C:\";
            ofd.Filter           = browserProperties.filter ?? "All files (*.*)|*.*";
        ...
        /// <summary>
        /// FileDialog for return file path
        /// </summary>
        /// <param name="browserProperties">Special Properties of File Dialog</param>
        /// <returns>Returns FileName(FilePath) string if selected valid file and pressed OK, or null</returns>
        public string OpenFileBrowser(BrowserProperties browserProperties)
        {
            var ofd = new VistaOpenFileDialog();
            ofd.Multiselect      = false;
            ofd.Title            = browserProperties.title ?? "Select a File";
            ...
            return ofd.ShowDialog(new WindowWrapper(GetActiveWindow())) == DialogResult.OK ? ofd.FileName : null;
        }
